...
var dateModule = {
  ...
  parse: function ( value, format ) {
    ...
    if ( val.y ) {
      val.y = (val.y > 30 ? '19' : '20' ) + val.y;
    }
    year = val.Y || val.y || today.getFullYear();

    if ( val.F ) {
      val.F = settings.indexOf(val.F);
    }
    if ( val.M ) {
      val.M = settings.indexOf(val.M);
    }
    month = val.F || val.M;
    ...
    day = val.d || val.j || today.getDate();
    hours = val.g || val.G || val.h || val.H;
    minutes = val.i;
    seconds = val.s;
